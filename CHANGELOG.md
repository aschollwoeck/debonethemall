...
- **M2b · Restyle, slice 8 (necromantic-sigil skill tree)** — rebuilt the Hub's skill tree from a
  ...
  concentric sigil rings. Node states read at a glance — **owned** (accent glow), **available**
  (amber pulse), **locked** (dim behind its gate), **reachable-but-unaffordable** (amber dim) — and
  a branch's veins **light up** as it's unlocked. Hovering a node shows an info-card (name / status
  / description); clicking an available node buys it. Purchasing, routing, currency and the
